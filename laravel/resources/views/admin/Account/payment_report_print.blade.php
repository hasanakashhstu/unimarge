					...
					@foreach($invoice as $invoice_data)
						@php
							...
							$invoice_component=DB::table('invoice_child')
							                 ...
							                 ->get();
							$waver_total=$waver_total+(int)$invoice_data->waber;                                    
						@endphp                 
			    <?php
...
<table style="margin-left: 200px;width: 75%;line-height: 0px;">
	 <tr>
          <th style="height:15px;width: 278px;background-color: #afafaf;color: black;"><?php echo @$i+1 ?></th>
           <th style="height:15px;background-color: #afafaf;color: black;">
           	<span style="margin-left: -235px;">Canteen</span></th>
    </tr>
    <tr>
    	<td colspan="3">
            		<table style="width: 100%;line-height: 0px;" border="1"  cellpadding="8">
            			<tr>
		            		<th style="width: 256px;">Month</th>
		            		<th style="width: 262px;">Fees</th>
		            		<th style="width: 410px;">Payment</th>
		            		<th>Due</th>
	            	   </tr>
						...
						@php
							$canteen_due=0;
							$total_canteen_fees=0;
							$total_canteen_payment=0;
						@endphp
						...
						@foreach($month as $key=>$value)
							...
							@php
								$canteen_month_fees=DB::table("invoice")
												->whereMonth('from_date', '=', $key+1)
												->where('student_roll',$data->roll)
												->where('form_name','canteen')
												->first();


							    $canteen_component=DB::table('roster_configuration')
							    						->first();

							    						
                                $canteen_component_fees=DB::table('payment_receipt')
                                                            ->join('payment_receipt_child','payment_receipt_child.payment_receipt_id','=','payment_receipt.payment_receipt_id')
                                                            ->where('student_roll',$data->roll)
                                                            ->where('payment_receipt_child.component_id',$canteen_component->default_component)
                                                            ->whereMonth('receipt_date','=',$key+1)
                                                            ->first();
							@endphp


							@if($canteen_month_fees)
							@php
								$canteen_fees=$canteen_month_fees->on_net_total
							@endphp
						    @else
								@php
									$canteen_fees=0
							...
							@if($canteen_component_fees)
								@php
									$canteen_component_fees_data=$canteen_component_fees->receipt_amount
								@endphp
							@else
								@php
									$canteen_component_fees_data=0
								@endphp
							@endif

						@php
						    $canteen_due=($canteen_due+$canteen_fees)-$canteen_component_fees_data;
							$total_canteen_fees=$total_canteen_fees+$canteen_fees;
							$all_payment_total=$all_payment_total+$total_canteen_fees;
							$all_fees_total=$all_fees_total+$total_canteen_fees;
							$total_canteen_payment=$total_canteen_payment+$canteen_component_fees_data;
						@endphp	
	                   ...
	                   <tr>
							...
							<td style="text-align: center;">{{$canteen_fees}}</td>
							<td style="text-align: center;">{{$canteen_component_fees_data}}</td>
							<td style="text-align: center;">{{$canteen_due}}</td>
						</tr>
					   @endforeach

					   	<tr>
	            	   	   <td style="text-align: center;background-color: #f5f5f5;color: #0404b1;">Sub Total</td>
	            	   	   <td style="text-align: center;background-color: #e2e2e2;">{{$total_canteen_fees}}</td>
	            	   	   <td style="text-align: center;background-color: #e2e2e2;">{{$total_canteen_payment}}</td>
	            	   	   <td style="text-align: center;background-color: #e2e2e2;">{{$canteen_due}}</td>
						</tr>
	    </table>
    </tr>
</table>
@php
$in_total_waver=$all_fees_total-$waver_total-$all_payment_total;
...
<table style="margin-left: 200px;width: 75%;line-height: 0px;" border="1"  cellpadding="8">
    ...
    font-weight: 700;">{{$waver_total}}</td>
