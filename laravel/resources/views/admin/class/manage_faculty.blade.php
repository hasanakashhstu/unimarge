...
@section('content')
    ...
    <div class="container">
        <h2><i class="fa fa-plus-circle" aria-hidden="true"></i> Manage Faculty</h2> <!-- Tab Heading  -->
        <p title="Transport Details">{{ Session::get('school.system_name') }}( {{ Session::get('school.school_eiin') }}
            )
            Faculty Details</p><br /><!-- Transport Details -->
        ...
        <ul class="nav nav-tabs">
            ...
            @can('create department')
                <li><a data-toggle="tab" href="#menu1"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add New Faculty</a>
                </li>
            @endcan
        </ul>
                ...
                <div class="widget-box">
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered data-table">
                            ...
                            @foreach ($faculty as $faculty_list)
                                <tr class="gradeX">
                                    {{-- <td>{{$department_list->class_name}}</td> --}}
                                    <td>{{ $faculty_list->faculty_code }}</td>
                                    <td>{{ $faculty_list->faculty_name }}</td>
                                    <td>{{ optional($faculty_list->getChairperson)->teacher_name }}</td>
                                    <td>{{ $faculty_list->description }}</td>
                                    <td>{{ $faculty_list->created_at }}</td>
                                    <td>{{ $faculty_list->updated_at }}</td>
                                    <td id="my_align" class="display_status">
                                        @can('edit department')
                                            {{ Form::open(['url' => url('manage_faculty/' . $faculty_list->faculty_id . '/edit'), 'method' => 'GET']) }}
                                            {{ Form::submit('Edit', ['class' => 'btn btn-primary']) }}
                                            {{ Form::close() }}
                                        @endcan

                                        @can('delete department')
                                            {{ Form::open(['url' => "/manage_faculty/$faculty_list->faculty_id", 'method' => 'DELETE']) }}
            ...
            <div id="menu1" class="tab-pane fade">
                <div>
                    <div class="widget-box">
                        ...
                        <div class="widget-content nopadding">
                            {{ Form::open(['url' => '/manage_faculty', 'class' => 'form-horizontal', 'method' => 'post', 'name' => 'basic_validate', 'id' => 'basic_validate', 'novalidate' => 'novalidate']) }}

                            <div class="control-group">
                                {{ Form::label('Faculty Name', '', ['class' => 'control-label']) }}
                                <div class="controls">
                                    {{ Form::text('faculty_name', '', ['title' => 'faculty_name']) }}
                                </div>
                            </div>
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    {{ Form::text('faculty_code', '', ['title' => 'faculty_code']) }}
                                </div>
                            </div>
