            ...
            <div class="row justify-content-center">
                <div class="col-lg-9 col-12 pd-10">
                    <table class="table table-hover">
                        ...
                        @if ($newsList->count() > 0)
                            @foreach ($newsList as $news)
                                <tr>
                                    ...
                                    <td>
                                        <h2> <a
                                                href="{{ url('news/' . $news->website_news_id) }}">{{ str_limit($news->title, 150) }}</a>
                                        </h2>
                                        <p>{!! str_limit($news->description, 150) !!}</p>
                                    </td>
                                    <td class="text-align-center">
                                        <a class="btn btn-primary mt-2" href="{{ Storage::url($news->file) }}"
                                            download="{{ $news->title }}"><svg xmlns="http://www.w3.org/2000/svg"
                        ...
                        @else
                            <tr>
                                <td colspan="3" class="text-center text-danger">No News Available!</td>
                            </tr>
                        @endif
                    </table>
