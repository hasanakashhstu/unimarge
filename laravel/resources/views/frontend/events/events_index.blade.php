    ...
    <div class="page-title-area bg-overlay text-center coursetitle">
        <div class="container">
            <div class="breadcrumb-inner">
                <h2 class="page-title"> {{ ucfirst($type) }} Events</h2>
            </div>
        </div>
    </div>
    <!-- page title end -->
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        @foreach ($events as $event)
                                            <tr>
                                                ...
                                                <td>
                                                    <h2> <a
                                                            href="{{ url('events/' . $event->website_events_id) }}">{{ str_limit($event->title, 150) }}</a>
                                                    </h2>
                                                    <p>{!! str_limit($event->description, 150) !!}</p>
                                                </td>
                                                <td>{{ $event->getDepartment->department_name }}</td>
                                                <td class="text-align-center">
                                                    <a class="btn btn-primary mt-2"
                                                        href="{{ Storage::url($event->image) }}" download="{{ $event->title }}"><svg
